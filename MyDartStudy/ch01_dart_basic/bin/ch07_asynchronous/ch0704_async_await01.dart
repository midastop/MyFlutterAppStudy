...
/* future(소문자 "f")는 Future 클래스의 인스턴스라고 하자. 이 인스턴스는 비동기 작업의
 * 결과를 나타내며 미완료(uncompleted)와 완료(completed) 2가지 상태를 가진다. 미완료는
 ...
 * 문자열을 값을 생성한다. 만약 Future<void>라면 사용가능한 값을 생성하지 않는다.
 * Future 클래스의 이름에서와 같이 "미래"라는 의미대로 미래에 받을 값을 의미한다.
 *
 * Future<T>와 같이 제넥릭을 사용해 미래에 받을 값의 데이터 타입을 지정해 사용하면 된다.
...
void main() async {
  // await은 Future<T>를 반환하는 함수에 적용할 수 있다.
  await order();
  await cooking();
  complete();
}
